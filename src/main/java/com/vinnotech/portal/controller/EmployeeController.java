...
public class EmployeeController {
	...
	/**
	 * Getting attachments of employee
	 * 
	 * @param empId
	 * @return
	 */
	...
	public ResponseEntity<Attachments> getAttachments(@PathVariable Long empId) {
		...
		LOGGER.info(CLASSNAME + ": Entering into the " + methodName + " method");
		Attachments att = empService.getAttachments(empId);
		HttpHeaders header = new HttpHeaders();
		header.add("desc", "Getting attachments of employee");
		LOGGER.info(CLASSNAME + ": Existing from  " + methodName + " method");
		return ResponseEntity.status(HttpStatus.OK).headers(header).body(att);
	}
