...
public class CourseController {
	 ...
	 * @return
	 */
	@GetMapping("/searchcourse/{publish}/{serachParam}/{offset}/{pageSize}")
	public ResponseEntity<Page<Course>> getCoursesBySearchTerm(@PathVariable("publish") boolean publish,
		...
		String methodName = "getCoursesBySearchTerm";
		LOGGER.info(CLASSNAME + ": Entering into the " + methodName + " method");
		Page<Course> searchResult = courseService.searchAllCourse(publish, serachParam, offset, pageSize);
		HttpHeaders header = new HttpHeaders();
		header.add("desc", "getting All Courses by search term");
		LOGGER.info(CLASSNAME + ": Existing from  " + methodName + " method");
		return ResponseEntity.status(HttpStatus.OK).headers(header).body(searchResult);
	}
	 ...
	 */
	@PreAuthorize(HRPortalConstants.ROLE_ADMIN_HR_RECRUITER_ONLY)
	@GetMapping("/searchcoursehr/{serachParam}/{offset}/{pageSize}")
	public ResponseEntity<Page<Course>> getCoursesBySearchTermHr(@PathVariable("serachParam") String serachParam,
			@PathVariable("offset") int offset, @PathVariable("pageSize") int pageSize) {
		String methodName = "getCoursesBySearchTermHr";
		LOGGER.info(CLASSNAME + ": Entering into the " + methodName + " method");
		Page<Course> searchResult = courseService.searchAllCourseHr(serachParam, offset, pageSize);
		HttpHeaders header = new HttpHeaders();
		header.add("desc", "getting All Courses by search term for Hr");
		LOGGER.info(CLASSNAME + ": Existing from  " + methodName + " method");
		return ResponseEntity.status(HttpStatus.OK).headers(header).body(searchResult);
	}
}
